    ...
    public class Reader
    {
        ...
        public static void FeedScore(Dictionary<string,Dictionary<string,double>> Files, Dictionary<string,double> QW, Dictionary<string,double> Score)
        {
            ...
            foreach(var word in QW){
                d2+=Math.Pow(word.Value,2);
            }

            foreach(var file in Files){
                double numerador = 0, d1 = 0, score = 0;
                foreach(var word in QW){
                    if(Files[file.Key].ContainsKey(word.Key)){
                        numerador+=QW[word.Key]*Files[file.Key][word.Key];
                    }
                }

                foreach(var word in file.Value){
                    d1+=Math.Pow(word.Value,2);
                }
                score = numerador/(Math.Sqrt(d1)*Math.Sqrt(d2));
        ...
        // Fin de la recolecta de datos del Modelo Vectorial
